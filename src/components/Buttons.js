...
export const SecondaryButton = ({ children, className, ...rest }) => {
  return (
    <Link
      {...rest}
      className={`inline-flex items-center justify-center px-4 py-2 border border-transparent text-xs font-medium transition-colors duration-100 text-black bg-stone-200 rounded-md shadow lg:text-sm group hover:bg-white ${className}`}
    >
      {children}
    </Link>
  )
